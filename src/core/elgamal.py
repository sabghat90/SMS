...
if __name__ == "__main__":
    print("=== ElGamal & KDC Module Tests ===\n")
    ...
    print(f"   Alice's keys generated:")
    print(f"   - Prime (p): {alice_keys.p}")
    print(f"   - Generator (g): {alice_keys.g}")
    print(f"   - Private key (x): {alice_keys.private_key}")
    print(f"   - Public key (y): {alice_keys.public_key}\n")
    
    # Test encryption/decryption with small integer
    print("2. Encrypt/Decrypt Integer:")
    message = 12345
    ciphertext = ElGamal.encrypt(message, alice_keys)
    decrypted = ElGamal.decrypt(ciphertext, alice_keys)
    print(f"   Original: {message}")
    print(f"   Ciphertext: {ciphertext}")
    print(f"   Decrypted: {decrypted}")
    print(f"   Match: {message == decrypted}\n")
    
    # Test encryption/decryption with string (short)
    print("3. Encrypt/Decrypt Short String:")
    message = "Hi"
    ciphertext = ElGamal.encrypt(message, alice_keys)
    decrypted = ElGamal.decrypt_to_string(ciphertext, alice_keys)
    print(f"   Original: {message}")
    print(f"   Ciphertext: {ciphertext}")
    print(f"   Decrypted: {decrypted}\n")
    
    # Test Key Distribution Center
    print("4. Key Distribution Center (KDC):")
    ...
    print(f"   Registered users: {kdc.list_registered_users()}")
    
    # Alice retrieves Bob's public key
    bob_public_key = kdc.get_public_key("bob")
    print(f"   Alice retrieved Bob's public key: {bob_public_key.public_key}")
    
    # Alice encrypts message for Bob
    message_for_bob = 9999
    encrypted_for_bob = ElGamal.encrypt(message_for_bob, bob_public_key)
    
    # Bob decrypts message
    decrypted_by_bob = ElGamal.decrypt(encrypted_for_bob, bob_keys)
    print(f"   Alice -> Bob: {message_for_bob}")
    print(f"   Bob decrypted: {decrypted_by_bob}")
    print(f"   Match: {message_for_bob == decrypted_by_bob}\n")
